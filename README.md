> **Note:** `main` contains an early prototype of this project (single-board sketch, no LoRa mesh integration). The code, dataset, and firmware actually used in the paper *"Dynamic Weight Quantization for Embedded Federated Learning in LoRa Mesh Networks: A Case Study"* are on the **[`no-reliable-payloads`](https://github.com/NilLlisterri/FL_LoRaMesher/tree/no-reliable-payloads)** branch:
> ```sh
> git checkout no-reliable-payloads
> ```
> The description below documents that branch.
