...
std::vector<uint16_t> getRoutingTable() {
  ...
  // if (debugEnabled) Serial.println("[M4] Getting routing table");
  Serial1.write('r'); // Send command
  while (!Serial1.available()) {
    // if (debugEnabled) Serial.println("[M4] Waiting for nodes count");
    delay(500);
  }
  uint8_t count = Serial1.read();
  // if (debugEnabled) Serial.println("[M4] Number of nodes: " + String(count));
  for (int i = 0; i < count; i++) {
    while (Serial1.available() < 2) {
      // if (debugEnabled) Serial.println("[M4] Waiting for node address");
      delay(500);
    }
    uint16_t node;
    Serial1.readBytes((char*)&node, 2);
    nodes.push_back(node);
    // if (debugEnabled) Serial.println("[M4] Node " + String(i + 1) + ": " + String(nodes[i]));
  }
  lock_modem = false;
  return nodes;
}
...
int getModemMessage(byte*& bytesPtr, uint16_t &recipient) {
  ...
  while (millis() < limit) {
    ...
    delay(250);
    if (debugEnabled) Serial.println("[M4] Waiting for modem message");
    if (Serial1.available()) {
      char command = Serial1.read();
      if (command != 'r') {
        Serial.println("[M4] Unknown command received " + String(command));
        while(true) {
          Serial.print(Serial1.read());
        }
      }
      if (debugEnabled) Serial.println("[M4] Received command: " + String(command));
      while (Serial1.available() < 2) {
        if (debugEnabled) Serial.println("[M4] Waiting for recipient");
        delay(100);
      }
      ...
      while (Serial1.available() < 2) {
        if (debugEnabled) Serial.println("[M4] Waiting for bytes count");
        delay(100);
      }
      ...
      if (debugEnabled) Serial.println("[M4] Bytes count: " + String(bytesCount));
      bytesPtr = (byte*) malloc(bytesCount);
      if (debugEnabled) Serial.println("[M4] Receiving bytes");
      for (int i = 0; i < bytesCount; i++) {
        while (!Serial1.available()) {
          if (debugEnabled) Serial.println("[M4] Waiting for byte " + String(i+1)); 
          delay(100); 
        }
        bytesPtr[i] = Serial1.read();
        // if (debugEnabled) Serial.println("[M4] Got byte " + String(i + 1) + " (int): " + String((int)bytesPtr[i]));
      }
      if (debugEnabled) Serial.println("[M4] Modem received " + String(bytesCount) + " bytes");
      return bytesCount;
    }
  }

  if (debugEnabled) Serial.println("[M4] Get modem message timeout");
  return 0;
}
...
int sendModemMessage(uint16_t recipient, uint16_t size, byte* bytes, bool expectResponse = false, byte*& response = defpointer) {
  bool received = false;
  while (!received) {
    if (debugEnabled) Serial.println("[M4] Attempting to send message");
...
std::map<uint16_t, uint16_t> getNodesEpochs() {
  ...
  for (uint i = 0; i < nodes.size(); i++) {
    byte data[1] = {'n'};
    byte* response;
    // if (debugEnabled) Serial.println("[M4] Sending message requesting epochs");
...
std::vector<float> getNodeWeights(uint16_t node, int batchNum) {
  lock_modem = true;
  if (debugEnabled) Serial.println("[M4] Requesting batch " + String(batchNum));
  ...
  if (debugEnabled) Serial.println("[M4] Batch " + String(batchNum) + " received, responseLength: "+ String(responseLength));
  std::vector<float> weights;
...
void sendWeights(uint16_t recipient, uint16_t batchNum) {
  if (debugEnabled) Serial.println("[M4] Received weights request for batch " + String(batchNum) + " from " + String(recipient));
  std::vector<float> weights = getWeightsValues(batchNum); // RPC.call("getWeightsValues", batchNum).as<std::vector<float>>();
  if (debugEnabled) Serial.println("[M4] Received " + String(weights.size()) + " weights from getWeightsValues, batch size is: " + String(weights.size()));
  float weightsArr[batchSize];
  for(uint i = 0; i < weights.size(); i++) {
    weightsArr[i] = weights[i];
  }
  if (debugEnabled) Serial.println("[M4] Sending batch...");
  sendModemMessage(recipient, sizeof(weights), (byte*) weightsArr);
  Serial.println("[M4] Batch sent!");
}

void sendNewSamplesCount(uint16_t recipient) {
  if (debugEnabled) Serial.println("[M4] Received samples count request, returning " + String(num_epochs));
  byte data[2];
...
void loop() {
  ...
  if (!lock_modem && Serial1.available()) { // Mesagge from LoRaMesher
    if (debugEnabled) Serial.println("[M4] Modem message is available!");
    byte* bytes;
    uint16_t recipient;
    int bytesCount = getModemMessage(bytes, recipient);

    if (debugEnabled) Serial.println("[M4] Received " + String(bytesCount) + " bytes from modem");
